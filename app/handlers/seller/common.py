...
from app.storage import is_seller, get_seller, get_all_products
...
async def profile_handler(message: Message):
    ...
    if seller:
        role = "🏪 Seller"
        extra = f"\n🏪 Do'kon: {seller['shop_name']}\n📦 Mahsulotlar: {len(get_seller_products(user.id))} ta"
    else:
        role = "🛍 Xaridor"
...
async def market_handler(message: Message):
    ...
    for p in products[:10]:  # max 10 ta ko'rsat
        text = (
            f"📦 <b>{p['name']}</b>\n"
...
def get_seller_products(seller_id):
    from app.storage import get_seller_products as _gsp
    return _gsp(seller_id)
